...
本文書は、**requirements.md** で定義された要件を技術的に実現するための設計を定義する。

| 文書 | 役割 |
|------|------|
| requirements.md | 何を作るか（WHAT）、なぜ作るか（WHY） |
| **design.md（本文書）** | どう作るか（HOW） |
| tasks.md | どの順序で実装するか（WHEN/ORDER） |
...
| 要件ID | 要件名 | 対応セクション |
|--------|--------|--------------|
| FR-001 | 画像アップロード | §6 API仕様, §7 Web UI仕様 |
| FR-002 | SIFT特徴点マッチング | §5.1 SIFT特徴量マッチング |
| FR-003 | ホモグラフィ推定 | §5.2 ホモグラフィ推定 |
| FR-004 | 画像ブレンディング | §5.4 ワープ変換とブレンディング |
| FR-005 | 進捗表示 | §6.5 SSE仕様 |
| FR-006 | 結果出力 | §8 入出力仕様 |
| FR-007 | Web UI | §7 Web UI仕様 |
| FR-008 | EXE配布 | §10 ファイル構成 |
| FR-009 | CLI実行 | §3 処理フロー |
| FR-010 | 部分失敗時継続 | §9 エラーハンドリング |
| FR-012 | 品質スコア表示 | §5.3 ホモグラフィ検証 |
...
- **Content-Type**: image/png
- **Content-Disposition**: attachment; filename="stitched_{job_id}_{timestamp}.png"
- **Body**: 合成結果画像のバイナリ
...
| キーポイント | 画像内の特徴的な点（コーナー、ブロブ等） |
| ディスクリプタ | キーポイント周辺の局所特徴を表すベクトル |
...
|---------|------|--------|---------|
| 1.0.0 | 2025-12-12 | - | 初版作成（SPECIFICATION.mdから再構成） |
...
| 文書 | パス | 説明 |
|------|------|------|
| 要件定義書 | specs/requirements.md | 本設計書が実現する要件 |
| タスク一覧 | specs/tasks.md | 本設計書に基づく実装タスク |
